...
function getSongIcon(link) {
  if (!link) return '♪';
  if (link.includes('spotify')) return '🎧';
  if (link.includes('youtube') || link.includes('youtu.be')) return '▶';
  return '♪';
}
      ...
      {songs && songs.length > 0 && (
        <div ref={songRef} className={`songs-wrap ${songInView ? 'visible' : ''}`}>
          <div className="songs-header">
            ...
            <p className="songs-hint">Bấm vào bài hát để nghe 🎧</p>
          </div>
          <div className="songs-list">
            {songs.map((s, i) => (
              <div
                key={i}
                className={`song-item ${s.link ? 'clickable' : ''}`}
              ...
              >
                ...
                <span className={`song-note ${s.link ? 'has-link' : ''}`}>
                  {getSongIcon(s.link)}
                </span>
              </div>
            ))}
